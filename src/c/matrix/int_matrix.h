#ifndef INT_MATRIX_H
#define INT_MATRIX_H

#include "matrix.h"

struct matrix* matrix_new_int(size_t width, size_t height);

long matrix_col_sum(struct matrix *matrix, size_t n);

long matrix_row_sum(struct matrix *matrix, size_t n);

void matrix_scalar_mult(struct matrix *matrix, int scalar);

struct matrix* matrix_mult(struct matrix *m1, struct matrix *m2);

struct matrix* matrix_sum(struct matrix *m1, struct matrix *m2);

#endif /* ! INT_MATRIX_H */
