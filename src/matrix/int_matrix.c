...
struct int_matrix *int_matrix_mult(struct int_matrix *m1, struct int_matrix *m2)
{
    ...
    for (size_t i = 0; i < m1->height; i++)
    {
        for (size_t j = 0; j < m2->width; j++)
        {
            for (size_t k = 0; k < m1->width; k++)
                res->data[i * m2->width + j] +=
                    m1->data[i * m1->width + k] * m2->data[k * m2->width + j];
        }
    }
